...
export class CreateEventDto {
    ...
    static validate(data: unknown): ValidationResult {
        ...
        if (!d.startTime || (typeof d.startTime !== 'string' && !(d.startTime instanceof Date))) {
            errors.push({
                field: 'startTime',
                message: 'startTime is required and must be a date string',
                value: d.startTime,
            });
        ...
        if (d.durationMin === undefined || typeof d.durationMin !== 'number' || d.durationMin < 1) {
            errors.push({
                field: 'durationMin',
                message: 'durationMin is required and must be a positive number',
                value: d.durationMin,
            });
        }
        ...
        if (
            ...
            typeof d.recurrenceRule !== 'string'
        ) {
            errors.push({
                field: 'recurrenceRule',
                message: 'recurrenceRule must be a string or null',
                value: d.recurrenceRule,
            });
        }
